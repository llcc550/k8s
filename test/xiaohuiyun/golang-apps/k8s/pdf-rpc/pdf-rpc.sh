...
serviceName="pdf-rpc"
fileName="pdf.go"
servicePath="src/epoch/app/ujy/service/pdf/cmd/rpc"
...
# 编译golang
docker run --rm -v "$(pwd)":/go/ -w /go/$servicePath -e CGO_ENABLED=1 -e GOOS=linux -e GOARCH=amd64 swr.cn-east-3.myhuaweicloud.com/wx-xiaoyang-public/golang:golang-1.17.2-imagemagick-7.1.0-1-ffmpeg-amd64 go build -a -installsuffix cgo -ldflags="-s -w" -o $serviceName $fileName
...
echo "FROM swr.cn-east-3.myhuaweicloud.com/wx-xiaoyang-public/golang:imagemagick-7.1.0-1-ffmpeg-amd64
COPY ttf /ttf
WORKDIR /app
