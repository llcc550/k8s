...
fileName="module.go"
servicePath="src/epoch/app/ujy/service/module/cmd/api"
...
echo "----------------------------k8s部署状态-----------------------------"
sleep 10
kubectl get pod -n $namespace -o wide | grep $serviceName | grep -v cloudscreen 
podStatus=`kubectl get pod -n $namespace | grep $serviceName | grep -v cloudscreen  | grep Running | awk '{print $1}' | head -n 1`
if [ ! $podStatus ] ; then
