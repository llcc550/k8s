...
# 项目配置文件所在目录
configFileDir="/data/code/config/test/zaun/certificate-web"
# 源代码所在目录
sourceCodeDir="/data/code/certificate-web"

echo "----------------------------certificate-web: 开始更新-----------------------------"
...
cd $configFileDir
docker build --no-cache -t $imageRegistry/zaun-certificate-web:$nowImageVersion .
docker push $imageRegistry/zaun-certificate-web:$nowImageVersion
...
echo "----------------------------k8s部署状态-----------------------------"
sleep 10
kubectl get pod -n $namespace -o wide | grep certificate-web
podStatus=`kubectl get pod -n $namespace | grep certificate-web  | grep Running | awk '{print $1}' | head -n 1`
if [ ! $podStatus ] ; then
  echo "----------------------------certificate-web: 更新失败-----------------------------"
else
  echo "----------------------------certificate-web: 更新成功-----------------------------"
fi