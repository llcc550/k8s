...
servicePath="src/zaun/app/certificate/cmd/api"
...
# 复制应用配置文件
/bin/cp -f /data/code/config/test/zaun/certificate-api/config.yaml /data/code/config/build/$serviceName.yaml
...
# 编译golang
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GOARM=6 go build -a -trimpath -installsuffix cgo -ldflags="-s -w" -o $serviceName certificate.go
...
k8sConfigFile="/data/code/config/test/zaun/certificate-api/deployment.yaml"
