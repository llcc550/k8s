...
serviceName="face-rmq"
fileName="face.go"
servicePath="src/demacia/service/face/rmq"
...
# 编译golang
CGO_ENABLED=1 GOOS=linux GOARCH=amd64 GOARM=6 go build -a -trimpath -installsuffix cgo -ldflags="-s -w" -o demacia-$serviceName $fileName
...
/bin/cp -f /data/code/config/build/libarcsoft_face/libarcsoft_face.so libarcsoft_face.so
/bin/cp -f /data/code/config/build/libarcsoft_face/libarcsoft_face_engine.so libarcsoft_face_engine.so
